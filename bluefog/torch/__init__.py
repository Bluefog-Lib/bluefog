...
# TODO(ybc) Update this distributed optimizer from Horovod
# gradient average style to weight/parameter average style.
...
class _DistributedConsensusOptimizer(torch.optim.Optimizer):
    ...
    def _neighbor_allreduce_data_async(self, p):
        name = self._parameter_names.get(p)
        handle = neighbor_allreduce_async(p.data, average=True, name=name)
        return handle
