...
def construct_topology(
    *,
    dst_list: Optional[List[int]] = None,
    src_list: Optional[List[int]] = None,
    construct_adjacency_matrix: bool = False
):
    """
    
    """
    if dst_list is None and src_list is None:
        raise ValueError("Either dst_list or src_list need to be provided.")
    if dst_list is not None and src_list is not None:
        raise ValueError(
            "Only one of two argument dst_list or src_list should be provided.")

    rank_list = dst_list or src_list
    print(bf.rank() in rank_list)
    is_valid, error_msg = _check_ranks(rank_list, bf.rank(), bf.size())
    assert is_valid, f"The format of dst_list or src_list is wrong: {error_msg}"

    degree = len(rank_list)
    all_degree_list = bf.allgather(torch.IntTensor([degree])).numpy()
    all_rank_list = bf.allgather(torch.IntTensor(rank_list)).numpy()
    adjacency_dict = dict()
    displacement = 0
