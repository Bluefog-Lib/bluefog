...
def _win_gossip_function_factory(tensor):
    return 'bluefog_torch_win_gossip_' + tensor.type().replace('.', '_')


def win_gossip_async(tensor: torch.Tensor, name: str, target_rank: int) -> int:
    function = _check_function(_win_gossip_function_factory, tensor)
    raise NotImplementedError
    # handle = getattr(mpi_lib, function)(tensor, name, target_rank)
    # _win_handle_map[handle] = name
    # return handle


def win_gossip(tensor: torch.Tensor, name: str, target_rank: int) -> bool:
    """ The basic gossip operator which makes the tensor (in-place) into the
    average value of tensor and the one in target rank.

    Please note this function is an atomic function for two processes and acquire
    the mutex underhood so that it may cause deadlock if you didn't use it properly.
    It is suggested to use biparte graph that one group of processes is active, who call
    this funciton and set the target_ranks belonges to the group of passive processes.

    Args:
        tensor: The tensor that used to take average with neighbor.
        name: The unique name to associate the window object.
        target_rank: The rank of target process to make average.

    Returns:
        A bool value to indicate the accumulate succeeded or not.
    """
    handle = win_gossip_async(tensor, name, target_rank)
    return win_wait(handle)
